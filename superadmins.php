...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($name) || empty($correo) || empty($pass)) {
    echo "Por favor, llena todos los campos.";
  } else {
    // Insertar la nueva información en la base de datos
    $consulta = "INSERT INTO superadmins (name, correo, pass) VALUES ('$name', '$correo', '$pass')";
    $result = mysqli_query($conexion, $consulta);

    if ($result) {
      echo "<div class='message-container alert alert-success'><h3 class='message text-success'>SuperUser successfully registered</h3></div>";
      // Agrega un retraso de 3 segundos antes de redirigir
      echo "<script>setTimeout(function() { window.location.href = 'indexadmin.php'; }, 3000);</script>";
    } else {
      echo "Error al guardar el SuperUser: " . mysqli_error($conexion);
    }
  }
}
      ...
      <div class="row full-height justify-content-center">
        <div class="col-12 text-center align-self-center py-5">
          <div class="section pb-5 pt-5 pt-sm-2 text-center">
            <h1 class="mb-0 pb-3"><span>Sign Up</span></h1>
                  ...
                  <div class="center-wrap">
                    <div class="section text-center">
                      <div class="form-group">
                        <input type="text" class="form-style" placeholder="Full Name" name="name" id="name">
                        <i class="input-icon uil uil-user"></i>
                      </div>
                      <div class="form-group mt-2">
                        <input type="email" class="form-style" placeholder="Email" name="correo" id="correo">
                        <i class="input-icon uil uil-at"></i>
                      </div>
                      <div class="form-group mt-2">
                        <input type="password" class="form-style" placeholder="Password" name="pass" id="pass">
                        <i class="input-icon uil uil-lock-alt"></i>
                      </div>
                      <button type="submit" class="btn mt-4" name="guardar3">Register</button>
                      <a href="indexadmin.php" class="btn mt-4">Go Back</a>
                    </div>
                  </div>
                </div>
              </div>
  ...
  <script>
    $(document).ready(function () {
      $("#registrationForm").submit(function (e) {
        ...
        // Si todos los campos están llenos y el correo tiene '@', envía el formulario
        this.submit();
      });
    });
  </script>
